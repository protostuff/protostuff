...
public class ArrayItemHasDelegateThrowNpeWhenDeserializingTest
{
    ...
    public void testIt()
    {
        try
        {
            DefaultIdStrategy idStrategy = new DefaultIdStrategy(
                    IdStrategy.DEFAULT_FLAGS | IdStrategy.COLLECTION_SCHEMA_ON_REPEATED_FIELDS);
            ...
            Schema<ClassThrowNpeWhenDeserializing> schema = RuntimeSchema.getSchema(
                    ClassThrowNpeWhenDeserializing.class, idStrategy);
            byte[] bytes = ProtobufIOUtil.toByteArray(originMsg, schema,
    ...
    public static class ProtobufCalendar implements Delegate<Calendar>
    {
        ...
        @Override
        public void writeTo(Output output, int number, Calendar value, boolean repeated)
                throws IOException
        {
        ...
        @Override
        public Class<Calendar> typeClass()
        {
